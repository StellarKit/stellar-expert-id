...
import Amount from './amount-view'
    ...
    switch (op.type) {
        ...
        case 'manageOffer':
            ...
            if (op.offerId && op.offerId !== '0') { //manage existing offer
                action = (isCancelled ? 'Cancel offer ' : 'Update offer ')
            } else {
                action = `Create offer`
            }
            if (isCancelled) {
                ...
                return <span>
                    <b>{action}</b> {op.offerId > 0 && ('#' + op.offerId)} sell <Amount amount={op.amount}
                                                                                        asset={op.selling}/> for <AssetLink
                    asset={op.buying} compact/> at <b>{op.price}
                    </b> {op.buying.code}/{op.selling.code}
                    <SourceAccount op={op} source={source}/>.
                </span>
            }
        case 'createPassiveOffer':
            return <span>
                    <b>Create passive offer</b> sell <Amount amount={op.amount} asset={op.selling}/> for <AssetLink
                asset={op.buying} compact/> at <b>{op.price}</b> {op.buying.code}/{op.selling.code}
                <SourceAccount op={op} source={source}/>.
                </span>
        case 'setOption':
        case 'setOptions':
