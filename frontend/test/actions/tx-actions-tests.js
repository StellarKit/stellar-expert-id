import responder from '../../ui/actions/responder'
...
describe('responder (tx-actions)', function () {
    ...
    before(function () {
        ...
        sinon.stub(axios, 'post').callsFake(function (url, payload) {
            const parsedUrl = new URL(url)
    ...
    async function confirmIntent(testCaseData, {expectedHorizonCalls, additionalCheck} = {}) {
        let account = loadAccount(),
            res = await responder.confirm(buildContext(testCaseData))
    ...
    it('fails to sign an invalid xdr', function () {
        return expect(responder.confirm(buildContext({intent: 'tx', xdr: '?'})))
